...
abstract: |
This project focuses on analyzing the difference in CPI(Consumer Price Index) from January 1947 to October 2023, using data retrieved from the Federal Reserve Bank of St. Louis, https://fred.stlouisfed.org/. The three major methods used in the analysis involve detrending, lag analysis, and incorporating economic variables into the model, notably, the economic variables fitting in the model including the unemployment rate, real consumption, real government spending, and real investment. The result indicates that the chosen economic variables fit in the model well. The regression model demonstrates a strong fit to the data, revealing direct relationships between economic variables and CPI. This result indicates the general relationship between CPI, the inflation rate, and the component of the inflation rate. The fitted CPI model is well corresponding to the real CPI, indicating the significance of the economic variables on CPI. The comprehensive analysis enhances our understanding of the role of economic variables in shaping CPI trends, providing a robust framework for future research in this domain.
...
```{r, echo=FALSE,results='hide'}
par(mfrow = c(1, 2))
plot(detrend_model$residuals, type = 'l', main = "Residuals of detrending model vs. Time", xlab="Time", ylab="Residuals")
pacf(detrend_model$residuals, main = "PACF of Residuals of detrending model", lag.max = 200)
```
...
plot(train[,'t'],fitted(model_1.1),  col = 'blue' , type = 'l', main = "Fitted CPI & real CPI vs. Time", xlab="Time", ylab="CPI")
